...
install:
	...
	if $(apparmor_parser --version | head -n1 | awk '{print $4}' | cut -c1) -lt 4
	then
		install --mode 644 src/maps-aa-profile-abi-4 $(DESTDIR)$(sysconfdir)/apparmor.d/usr.bin.maps	# change this later
	else
